...
def get_recommendations(followers, engagement_rate, campaign_roi, ad_cost, platform, location, industry, top_n=5):
    # Filter the dataset based on the selected platform, location, and industry
    ...
    # Create a dataframe with client preferences
    ...
    # Ensure client_encoded has the same columns as df_encoded_filtered
    for col in df_encoded_filtered.columns:
        if col not in client_encoded.columns:
            client_encoded[col] = 0

    # Reorder columns to match df_encoded_filtered
    client_encoded = client_encoded[df_encoded_filtered.columns]

    # Calculate similarity between client preferences and influencers from the filtered dataset
    client_similarity = cosine_similarity(client_encoded, df_encoded_filtered)

    # Get top N similar influencers
    similar_indices = client_similarity[0].argsort()[::-1][:top_n]
    return df_filtered.iloc[similar_indices]  # Return the top N recommendations
...
def index():
    ...
    return render_template('index.html', platforms=platforms, locations=locations, industries=industries)
...
def recommend():
    ...
    # Handle cases where recommendations are empty or None
